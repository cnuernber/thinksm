...
<xsl:stylesheet
  ...
  <xsl:template match="//@conf:datamodel">
    <xsl:attribute name="datamodel">clojure</xsl:attribute>
  </xsl:template>
  ...
  <xsl:template match="//@conf:compareIDVal"> 
	...
	<xsl:matching-substring>(<xsl:value-of select="regex-group(2)"/> Var<xsl:value-of select="regex-group(1)"/> Var<xsl:value-of select="regex-group(3)"/>)</xsl:matching-substring>
      </xsl:analyze-string>
    </xsl:attribute>
  </xsl:template> 
  ...
  <xsl:template match="//@conf:eventdataVal">
    <xsl:attribute name="cond">(= (:data event) <xsl:value-of select="."/>)</xsl:attribute>
  </xsl:template>
